...
class SimplexNoise {
  private:
    static glm::vec3 const directions[12];
    short const permutaion_table[256] = {
    ...
    double skew_factor;
    double unskew_factor;
    double F2 = 0.5 * (sqrt(3.0) - 1.0);
    double G2 = (3.0 - sqrt(3.0)) / 6.0;
    short permutations[512];
    short permutations_mod_twelve[512];

    void fill_permutations() {
        for (int i = 0; i < 512; i++) {
            permutations[i] = permutaion_table[i & 255];
            permutations_mod_twelve[i] = (short)permutations[i] % 12;
        }
    }
    double dot(glm::vec3 g, double x, double y, double z);
    double dot(glm::vec3 g, double x, double y);

  public:
    SimplexNoise();
    double noise_3D(double xin, double yin, double zin);
    double noise_2D(double xin, double yin);
};
